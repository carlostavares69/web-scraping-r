...
cols_relevantes <- c("ID","AIS","MUNICIPIO_HOMICIDIO","NATUREZA_HOMICIDIO","ARMA_UTILIZADA","DATA_HOMICIDIO","SEXO","IDADE","INCIDENCIA_HOMICIDIO","MES_ANO","FAIXA_ETARIA","GRUPO_NATUREZA_HOMICIDIO","GRUPO_MUNICIPIO","GRUPO_AIS","ARMA_DE_FOGO","POPULACAO","IDHM","PIB_PERCAPITA","LATITUDE","LONGITUDE")
...
cols_relevantes <- c("ID","AIS","MUNICIPIO_HOMICIDIO","NATUREZA_HOMICIDIO","ARMA_UTILIZADA","DATA_HOMICIDIO","SEXO","IDADE","INCIDENCIA_HOMICIDIO_BAIRRO","MES_ANO","FAIXA_ETARIA","GRUPO_NATUREZA_HOMICIDIO","ARMA_DE_FOGO","BAIRRO","LATITUDE","LONGITUDE")
...
#setdiff(df_raspagem$NOME_VITIMA, df_CSAI_SSPDS$NOME_VITIMA)
inexistente <- setdiff(df_CSAI_SSPDS$NOME_VITIMA, df_raspagem$NOME_VITIMA) %>% dplyr::data_frame(NOME_VITIMA = .)
inexistente 
...
for(i in 1:nrow(df_CSAI_SSPDS)) {
  ...
  if(!is.na(indice_linha[1])) {
      if(length(indice_linha[1]) > 0 ) {
        ...
        #df_raspagem[indice_linha[1], 2] <- df_CSAI_SSPDS[i, 1]$AIS
      ...
      # if(df_CSAI_SSPDS[i, 7]$NOME_VITIMA %in% inexistente$NOME_VITIMA) {
      # 
      #   print(paste("NOVA Linha para ->", df_CSAI_SSPDS[i,7]$NOME_VITIMA, sep = " "))
      # 
      #   df_raspagem <- rbind(
      #     dplyr::data_frame(
      # 
      ...
      # 
      #     ), df_raspagem
      #   ) %>% as.data.frame(.) #%>% arrange(as.Date(DATA_HOMICIDIO, format="%d/%m/%Y"))
      # 
      # }
