...
#
# Script:      Funcao_limpa_dados_2014.R - Funcoes necessarias para limpeza dos dados de 2014.
# Autor:       Erivando Sena
  ...
  for (num_doc in 1:nrow(df_nome_docs)) {
    ...
    if(file.exists(arquivo)) {
      if (num_doc == 1) {
        ...
        print(paste("########## Extraindo tabelas de", nome, "contendo", nrow(m_tabela), "linhas. ##########", sep = " "))
      ...
      if (num_doc == 5) {
        ...
        areas_tabela <- list(c("top" = 140.04757, "left" = 10.92779, "bottom" = 563.10924, "right" = 816.46219))
      ...
      if (num_doc == 11) {
        ...
        for (n_linha in 1:length(m_tabela)) {
          if(n_linha == 15) {m_tabela[n_linha,3] <- c("JUAZEIRO DO NORTE")}
          if(n_linha %in% c(45, 46, 72, 76, 112, 122, 123, 133, 308, 326, 334, 335, 351, 373) ) 
            {
              m_tabela[n_linha,3] <- m_tabela[15,3] 
              }
          if(n_linha %in% c(137,138) ) {m_tabela[n_linha,3] <- c("FORTALEZA")}
          if(n_linha %in% c(139,140) ) {m_tabela[n_linha,3] <- c("MULUNGU")}
          if(n_linha == 144) {m_tabela[n_linha,3] <- c("LIMOEIRO DO NORTE")}
          if(n_linha == 191) {m_tabela[n_linha,3] <- c("JIJOCA DE JERICOACOARA")}
          if(n_linha %in% c(194, 233, 241)) {
              m_tabela[n_linha,3] <- c("SÃO GONÇALO DO AMARANTE") 
              }
          if(n_linha == 300) {m_tabela[n_linha,3] <- c("TABULEIRO DO NORTE")}
          if(n_linha == 303) {m_tabela[n_linha,3] <- c("NOVA JAGUARIBARA")}
          if(n_linha == 331) {m_tabela[n_linha,3] <- m_tabela[251,3]}
          if(n_linha == 337) {m_tabela[n_linha,3] <- m_tabela[191,3]}
          if(n_linha %in% c(332, 358)) {m_tabela[n_linha,3] <- m_tabela[1,3]}
          if(n_linha == 375) {m_tabela[n_linha,3] <- m_tabela[71,3]}
        }
        ...
        # Corrige variaveis de valores inconsistentes
        for (n_linha in 1:nrow(m_tabela)) {
          if(!is.na(m_tabela[n_linha,5]) & m_tabela[n_linha,5] == c("ARMA D FOGO")) {
            m_tabela[n_linha,5] <- c("ARMA DE FOGO")
          }
        }
